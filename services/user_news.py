...
def get_latest_news(user_id: str, categories: Optional[List[str]] = None, limit: int = 10) -> List[dict]:
    ...
    if articles:
        current_new_date = max(datetime.fromisoformat(article['date_published']) for article in articles)
        current_old_date = min(datetime.fromisoformat(article['date_published']) for article in articles)
        
        update_data = {'id': user_id}
        
        if newest_listened_date is None or current_new_date > datetime.fromisoformat(newest_listened_date):
            update_data['newest_listened_date'] = current_new_date.isoformat()
            logger.info(f"Updated newest_listened_date to: {current_new_date}")
        
        if oldest_listened_date is None or current_old_date < datetime.fromisoformat(oldest_listened_date):
            update_data['oldest_listened_date'] = current_old_date.isoformat()
            logger.info(f"Updated oldest_listened_date to: {current_old_date}")
...
def prepare_for_transport(article):
    if article:
        article["audio_summary"] = util.generate_audio_url_public(article["audio_summary"])
